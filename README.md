...
2.  Add a "closed" class to the 'menu' setting element (see below). Default is the first <code><ul></code>. This is the same class as the "hideclass" option (see below) so if you change that also change in CSS file
...
The hiding container. Either your <code><ul></code> or some surrounding element

default: First found <code><ul></code> within the wrapper
